...
```
┌─────────────────┬──────────────────────────────────────────────┬──────────────┐
│ 📋 战斗列表      │ 绝对时间   │ 相对时间│ 事件  │ NPC │ 法术│目标│ 🔍 事件详情   │
│                 ├──────────────────────────────────────────────┤              │
│ ▼ 巴拉丁监狱     │ 05/21 16:22│  0:03.1 │ CAST │Arga│流星│玩家│ 时间: 0:03.1 │
│   Argaloth      │ 05/21 16:22│  0:15.7 │ AURA │Arga│狂暴│ —  │ 施法者: ...  │
│                 │            │         │      │    │    │    │              │
│ [⏰ 手动] [↩ 重置] │            │         │      │    │    │    │              │
├─────────────────┤            │         │      │    │    │    │              │
│ 选择 NPC        │            │         │      │    │    │    │              │
...
| 位置 | 选项 |
|------|------|
| 任意行 | ⏱ 设为战斗开始时间 (T=0) — 以该事件为起点重新计算相对时间 |
| 事件列 | 只看该事件类型 / 不看该事件类型 |
| 法术列 | 只看该法术 / 不看该法术 |
...
点击"⏰ 手动设置开始时间"按钮，弹出对话框显示当前开始时间（UTC+8），可直接输入新时间：

```
输入新的战斗开始时间（UTC+8）
格式: MM/DD/YYYY HH:MM:SS.ffffff

当前: 05/21/2026 16:22:45.593800
```

- 输入格式错误会弹窗提示，不会崩溃
- 早于新开始时间的事件显示为负数（如 `-0:05.3`）
- 点击"↩ 重置"恢复自动检测的开始时间
- duration 自动重新计算为 `max(全部) - min(全部)`
