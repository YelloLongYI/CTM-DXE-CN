...
function module:OnInitialize()
    Colors = addon.db.profile.Colors
...
function module:Distribute(key,target)
	...
	if not data or key == "default" or GetNumRaidMembers() == 0 then
		return
	end
...
-- TRANSFERS
----------------------------------
local data -- making data local outside the Transfer function is probably bad programming but the OnAccept function was holding an upvalue so it had to be done
...
function module:Transfer(msg, dist, sender)
	...
	if type == "UPLOAD" then
        ...
        local success
		success, data = self:Deserialize(serialData)
		...
		if not pfl.AutoAccept then
			...
			if not StaticPopupDialogs[popupkey] then
				local STATIC_CONFIRM = {
					...
					OnAccept = function() self:DLCompleted(key,data) end,
...
function module:DLCompleted(key,data)
    addon:UnregisterEncounter(key)
	addon:RegisterEncounter(data)
	addon.RDB[key] = data
	addon:SendWhisperComm(Downloads[key].sender,"VersionBroadcast",key,data.version)
	addon:Print(format(L["%s download from %s complete"],Downloads[key].name,Downloads[key].sender))
	self:UnregisterComm(TRANSFER_PREFIX)
	wipe(Downloads[key])
end

function module:DLRejected(key)
	self:UnregisterComm(TRANSFER_PREFIX)
	wipe(Downloads[key])
end
