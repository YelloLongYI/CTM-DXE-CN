...
local PATTERN_OFFICIAL = "^(%d+)$"
local PATTERN_TWINSTAR_RELEASE = "^(%d+) | ([%d%.]+)$"
local PATTERN_TWINSTAR_BETA = "^(%d+) | ([%d%.]+) %- (%d+)$"
local PATTERN_TWINSTAR_BETA2 = "^(%d+) | ([%d%.]+) %- (%d+) %(Beta%)$"
local PATTERNS = {PATTERN_OFFICIAL, PATTERN_TWINSTAR_RELEASE, PATTERN_TWINSTAR_BETA, PATTERN_TWINSTAR_BETA2}

 function addon:ParseVersion(version)
    if version == NONE or version == "-1" then return 0 end
    local officialVersion, twinstarVersion, betaVersion
    version = tostring(version)
    for _,pattern in ipairs(PATTERNS) do
        if version:find(pattern) then
            officialVersion, twinstarVersion, betaVersion = version:match(pattern)
            break
        end
    end
    
    officialVersion = officialVersion or 0
    twinstarVersion = twinstarVersion or 0
    betaVersion = betaVersion or 0
    
    return officialVersion*10^6+twinstarVersion*10^4+betaVersion
end

local function isTextAddonVerion(text)
    if type(text) == "string" then
        return text:find(PATTERN_OFFICIAL) or 
               text:find(PATTERN_TWINSTAR_RELEASE) or
               text:find(PATTERN_TWINSTAR_BETA)
    else
        return false
    end
end
...
local function ColorCode(text)
    if type(text) == "string" then
        if isTextAddonVerion(text) then
                local theirVersion = addon:ParseVersion(text)
                local myVersion = addon:ParseVersion(addon.version)
    ...
    elseif type(text) == "number" then
        if text == NONE then
            ...
            local myVersion = EDB[value].version
            if myVersion > text then
                return RED..text.."|r"
            elseif myVersion < text then
                return BLUE..text.."|r"
            else
                return GREEN..text.."|r"
            end
...
local function sort_asc(a,b)
	if sort_index == 2 then
		local a2,b2 = a[2],b[2]
        a2 = addon:ParseVersion(format("%s",a2))
        b2 = addon:ParseVersion(format("%s",b2))
...
local function CreateHeader(content,column)
	...
	text:SetPoint("TOPRIGHT",header,"BOTTOMRIGHT",50,-3)
	rows[1] = text
...
local function OnRefreshVersionList(self)
	if self:IsShown() then
		for k,v in ipairs(RVS) do
			if type(v.versions[value]) == "table" then
                v[2] = v.versions[value].version or NONE
            else 
...
local function CreateWindow()
	window = addon:CreateWindow(L["Version Check"],320,295,true)
