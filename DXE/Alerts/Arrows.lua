...
do
    ...
    local HOLD_FILE = "Interface\\Addons\\DXE\\Textures\\arrow-here.tga"
	local NUM_CELLS = 108
	local NUM_COLUMNS = 9
	local CELL_WIDTH = 56
	...
	function prototype:SetAngle(d,dx,dy,rangeStay,action)
        if (action == "TOWARD" and d > rangeStay) or (action == "AWAY" and d < rangeStay) then
            self.t:Show()
            self.tHold:Hide()
            -- Calculate
	...
	local function OnUpdate(self,elapsed)
		...
		if self.elapsed > self.persist then
            ...
            if not (self.fx and self.fy) and not UnitIsVisible(self.unit) then
                if not self.hidden then self:SetArrowVisibility(false) end
                return
            else
            ...
            if self.cancelrange then
                if self.action == "TOWARD" and d <= self.cancelrange then self:Destroy() return end
                if self.action == "AWAY" and d >= self.cancelrange then self:Destroy() return end
            end
            
			self:SetAngle(d,dx,dy,self.rangeStay,self.action)
    ...
    function prototype:SetArrowVisibility(visibility)
	...
	function prototype:SetTarget(unit,persist,action,msg,spell,sound,fixed,xpos,ypos,range1,range2,range3,cancelrange,rangeStay)
        ...
        self.cancelrange = cancelrange
		self.fmt = (spell and spell.." " or "").."<|cffffff78%.0f|r> "..CN[unit]
		...
		self.range3 = range3
        self.rangeStay = rangeStay or (action == "AWAY" and range3+(range3-range2) or 0)
	...
	function CreateArrow(i)
        ...
        local tHold = self:CreateTexture(nil,"OVERLAY")
        tHold:SetTexture(HOLD_FILE)
        tHold:ClearAllPoints()
        tHold:SetPoint("TOPLEFT",self,7,0)
        tHold:SetPoint("BOTTOMRIGHT",self,-7,0)
        tHold:Hide()
        self.tHold = tHold
...
function module:AddTarget(unit,persist,action,msg,spell,sound,fixed,xpos,ypos,range1,range2,range3,cancelrange,rangeStay)
    ...
    if UnitExists(unit) then
		...
		for i,arrow in ipairs(frames) do
			if not arrow.unit then
				arrow:SetTarget(unit,persist,action,msg,spell,sound,fixed,xpos,ypos,range1,range2,range3,cancelrange,rangeStay)
				break
			end
		end
	end
